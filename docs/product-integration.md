...
本轮只读查看本机五个产品工作区，没有修改这些仓库，也没有调用产品生产业务接口。部分工作区包含既有未提交或未跟踪内容，因此下列 HEAD 用于定位基线，不表示所有核查内容均已提交、发布或部署。
...
需补齐：固定三项接口的 data 字段与分页契约，取得部署 endpoint 和独立测试凭据，核验 key 被撤销/越权时的响应。把 CinaAuth 登录接入这些业务 API属于后续服务端工作；现有后台 SSO 的专用桥接密钥不能交给 CLI。
...
需补齐：核验生产开放接口启用状态、只读账号授权、分页边界、字段脱敏和 token 刷新语义。后续若支持多店铺/供应商，先明确服务端资源隔离契约。
...
需补齐：确定受支持的 RPC endpoint 和速率限制，固定网络元数据，验证测试网链状态查询。当前源码配置是测试网，不代表已经支持主网。
